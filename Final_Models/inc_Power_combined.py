...
print("script started: Combined Power Model Comparison")
...
def random_pos_uav(cx, cy):
    theta = 2 * np.pi * np.random.uniform()
    r     = R_0 + (R_max - R_0) * np.sqrt(np.random.uniform())
    return [cx + r*np.cos(theta), cy + r*np.sin(theta), UAV_Height]
...
def performance_metrics(sinr, is_uav, acc, P_total_UAV, P_tx_Host):
    ...
    if is_uav:
        ...
        Cap = 0.5 * np.log2(1 + sinr)  # consistent half-duplex
...
for model_name, params in MODELS.items():
    ...
    power_increment = max_power_host / Iteration_range

    for idx in range(1, Iteration_range + 1):
        acc_gen.reset(); acc_wor.reset(); acc_bes.reset(); acc_bl.reset()

        P_tx_Host = idx * power_increment
        results[model_name]['power_values'].append(P_tx_Host)
        print(f"  {model_name} | Power {P_tx_Host:.4f} W ({idx}/{Iteration_range})", end='\r')
        ...
        for j in range(num_iterations):
            ...
            # --- Spawn positions ---
            if is_disaster:
                UAV_pos_list  = [random_pos_uav(serving_x, serving_y) for _ in range(num_UAV)]
                ...
                UAV_pos_list  = [random_pos_uav(BS_Pos[servicing_BS][0], BS_Pos[servicing_BS][1])
            ...
            performance_metrics(SINR_gen, True,  acc_gen, P_total_UAV, P_tx_Host)
            performance_metrics(SINR_wor, True,  acc_wor, P_total_UAV, P_tx_Host)
            performance_metrics(SINR_bes, True,  acc_bes, P_total_UAV, P_tx_Host)
            performance_metrics(SINR_bl,  True,  acc_bl,  P_total_UAV, P_tx_Host)
        ...
        r['Out_baseline'].append(acc_bl.outage_sum / num_iterations)
...
def print_results(model_name):
    r       = results[model_name]
    x_vals  = r['power_values']

    # Use per-step baseline lists instead of grand averages
    bl_sinr = r['SINR_baseline']
    bl_cap  = r['Cap_baseline']
    bl_ber  = r['BER_baseline']
    bl_out  = r['Out_baseline']           # outage baseline: use UAV general as reference floor

    # Effective: baseline until UAV general is better
    eff_SINR = [max(s, b) for s, b in zip(r['SINR'], bl_sinr)]
    eff_Cap  = [max(c, b) for c, b in zip(r['Cap'],  bl_cap)]
    eff_BER  = [min(b1, b2) for b1, b2 in zip(r['BER'], bl_ber)]
    eff_Out  = [min(o, b)   for o, b  in zip(r['Out'], bl_out)]

    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle(f'{model_name} Model — UAV Relay Performance vs Host Transmit Power', fontsize=13)

    # ── SINR ─────────────────────────────────────────────────────────────────
    axes[0, 0].plot(x_vals, r['SINR'],     'b-o', markersize=3, label='General')
    axes[0, 0].plot(x_vals, r['SINR_wor'], 'r--', markersize=3, label='Worst')
    axes[0, 0].plot(x_vals, r['SINR_bes'], 'g--', markersize=3, label='Best')
    axes[0, 0].set_xlabel('Host Tx Power (W)'); axes[0, 0].set_ylabel('Mean SINR (linear)')
    axes[0, 0].set_title(f'Increasing Host Transmit Power {model_name} Mean SINR'); axes[0, 0].legend(); axes[0, 0].grid(True)

    # ── Capacity ──────────────────────────────────────────────────────────────
    axes[0, 1].plot(x_vals, r['Cap'],         'b-o', markersize=3, label='General')
    axes[0, 1].plot(x_vals, bl_cap,          'y-o', markersize=3, label='Baseline (direct link)')
    axes[0, 1].plot(x_vals, r['Cap_wor'],    'r--', markersize=3, label='Worst')
    axes[0, 1].plot(x_vals, r['Cap_bes'],    'g--', markersize=3, label='Best')
    axes[0, 1].set_xlabel('Host Tx Power (W)'); axes[0, 1].set_ylabel('Capacity (bits/s/Hz)')
    axes[0, 1].set_title(f'Increasing Host Transmit Power {model_name} Channel Capacity'); axes[0, 1].legend(); axes[0, 1].grid(True)

    # ── Outage ────────────────────────────────────────────────────────────────
    axes[0, 2].plot(x_vals, r['Out'],         'b-o', markersize=3, label='General')
    axes[0, 2].plot(x_vals, bl_out,          'y-o', markersize=3, label='Baseline (direct link)')
    axes[0, 2].plot(x_vals, r['Out_wor'],    'r--', markersize=3, label='Worst')
    axes[0, 2].plot(x_vals, r['Out_bes'],    'g--', markersize=3, label='Best')
    axes[0, 2].set_xlabel('Host Tx Power (W)'); axes[0, 2].set_ylabel('Outage Probability')
    axes[0, 2].set_title(f'Increasing Host Transmit Power {model_name} Outage Probability'); axes[0, 2].legend(); axes[0, 2].grid(True)

    # ── BER ───────────────────────────────────────────────────────────────────
    axes[1, 0].semilogy(x_vals, r['BER'],         'b-o', markersize=3, label='General')
    axes[1, 0].semilogy(x_vals, bl_ber,          'y-o', markersize=3, label='Baseline (direct link)')
    axes[1, 0].semilogy(x_vals, r['BER_wor'],    'r--', markersize=3, label='Worst')
    axes[1, 0].semilogy(x_vals, r['BER_bes'],    'g--', markersize=3, label='Best')
    axes[1, 0].set_xlabel('Host Tx Power (W)'); axes[1, 0].set_ylabel('BER')
    axes[1, 0].set_title(f'Increasing Host Transmit Power {model_name} BER (log scale)'); axes[1, 0].legend(); axes[1, 0].grid(True, which='both')

    # ── Energy Efficiency ─────────────────────────────────────────────────────
    axes[1, 1].plot(x_vals, r['EE'],     'b-o', markersize=3, label='General')
    axes[1, 1].plot(x_vals, r['EE_wor'], 'r--', markersize=3, label='Worst')
    axes[1, 1].plot(x_vals, r['EE_bes'], 'g--', markersize=3, label='Best')
    axes[1, 1].set_xlabel('Host Tx Power (W)'); axes[1, 1].set_ylabel('Energy Efficiency (bits/s/Hz/W)')
    axes[1, 1].set_title(f'Increasing Host Transmit Power {model_name} Energy Efficiency'); axes[1, 1].legend(); axes[1, 1].grid(True)

    # ── Effective SINR handover ───────────────────────────────────────────────
    axes[1, 2].plot(x_vals, eff_SINR, 'b-o', markersize=3, label='UAV SINR (General)')
    axes[1, 2].plot(x_vals, bl_sinr,  'y-o', markersize=3, label='Baseline (direct link)')
    axes[1, 2].set_xlabel('Host Tx Power (W)'); axes[1, 2].set_ylabel('Mean SINR (linear)')
    axes[1, 2].set_title(f'Increasing Host Transmit Power {model_name} Effective SINR ')
    axes[1, 2].legend(); axes[1, 2].grid(True)

    plt.tight_layout()
    plt.savefig(rf'C:\Users\n70943sb\OneDrive - The University of Manchester\uni\Y3\Individual Project\Results\{model_name}_plots_incPower.png')
    plt.show()
...
def plot_model_comparison():
    ...
    fig.suptitle('General SINR Comparison Across Host Transmit Power', fontsize=13)

    for model_name, r in results.items():
        s      = styles[model_name]
        bl_avg = float(r['baseline_total'] / (num_iterations * Iteration_range))
        eff_SINR = [max(val, bl_avg) for val in r['SINR']]

        ax.plot(r['power_values'], r['SINR'], color=s['color'], marker=s['marker'],
                markersize=3, linestyle='-', label=f'{model_name} SINR')

    ax.set_xlabel('Host Tx Power (W)')
    ax.set_ylabel('Mean SINR (linear)')
    ax.legend()
    ax.grid(True)

    plt.tight_layout()
    plt.savefig(rf'C:\Users\n70943sb\OneDrive - The University of Manchester\uni\Y3\Individual Project\Results\combined_SINR_plot_incPower.png')
