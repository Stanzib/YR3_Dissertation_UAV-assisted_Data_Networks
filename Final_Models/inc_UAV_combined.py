...
class PerfAccumulator:
    ...
    SINR_samples:   list = None
    Capacity_samples: list = None
    BER_samples:    list = None
    EE_samples:     list = None
...
def performance_metrics(sinr, is_uav, acc, P_total_UAV):
    ...
    acc.BER_sum        += BER
    acc.SINR_samples.append(sinr)
    # acc.Capacity_samples.append(Cap)
    # acc.BER_samples.append(BER)
    # acc.EE_samples.append(EE)
...
# Storage: results[model_name][metric] = list over UAV counts
results = {m: {
...
for model_name, params in MODELS.items():
    ...
    # Fixed interferer lists for urban/ideal (disaster uses build_fixed_interf_ring)
    if not is_disaster:
    ...
    for num_UAV in range(1, num_UAVs_total + 1):
        ...
        print(f"  {model_name} | UAV {num_UAV}/{num_UAVs_total}", end='\r')
        ...
        for j in range(num_iterations):
            ...
            # --- Spawn positions ---
            if is_disaster:
                UAV_pos_list  = [random_pos_uav(serving_x, serving_y, UAV_H) for _ in range(num_UAV)]
                ...
                UAV_pos_list  = [random_pos_uav(BS_Pos[servicing_BS][0], BS_Pos[servicing_BS][1], UAV_H)
            ...
            performance_metrics(SINR_bl,  False, acc_bl,  P_total_UAV)
            results[model_name]['baseline_total'] += SINR_bl

        r = results[model_name]
        r['SINR'].append(acc_gen.SINR_sum / num_iterations)
        #r['SINR'].append(float(np.median(acc_gen.SINR_samples)))
        ...
        #r['Cap'].append(float(np.median(acc_gen.Capacity_samples)))
        ...
        #r['BER'].append(float(np.median(acc_gen.BER_samples)))
        ...
        #r['EE'].append(float(np.median(acc_gen.EE_samples)))
        ...
        #r['SINR_baseline'].append(float(np.median(acc_bl.SINR_samples)))
...
def print_results(model_name):
    ...
    bl_avg    = float(r['baseline_total'] / (num_iterations * num_UAVs_total))

    # Baseline per-UAV-count averages for outage and BER
    bl_outage = r['SINR_baseline']  # already per-iteration average
    bl_cap_avg = float(sum(r['Cap_baseline']) / len(r['Cap_baseline']))
    bl_ber_avg = float(sum(r['BER_baseline']) / len(r['Cap_baseline']))
    # Build effective (combined) series — baseline until UAV general exceeds it
    eff_SINR   = [max(s, bl_avg)                          for s in r['SINR']]
    eff_Cap = [max(c, bl_cap_avg) for c in r['Cap']]
    # For outage and BER: UAV relay is better when its value is LOWER
    bl_out_avg = sum(r['Out']) / len(r['Out'])   # rough flat reference
    eff_Out    = [min(o, bl_out_avg)  for o in r['Out']]
    eff_BER    = [min(b, bl_ber_avg)  for b in r['BER']]

    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle(f'{model_name} Model — UAV Relay Performance vs Number of UAVs', fontsize=13)

    # ── SINR ─────────────────────────────────────────────────────────────────
    axes[0, 0].plot(uav_range, r['SINR'],      'b-o',  markersize=3, label='General')
    axes[0, 0].plot(uav_range, r['SINR_wor'],  'r--',  markersize=3, label='Worst')
    axes[0, 0].plot(uav_range, r['SINR_bes'],  'g--',  markersize=3, label='Best')
    axes[0, 0].set_xlabel('Number of UAVs'); axes[0, 0].set_ylabel('Mean SINR (linear)')
    axes[0, 0].set_title(f'Increasing UAV {model_name} Mean SINR'); axes[0, 0].legend(); axes[0, 0].grid(True)

    # ── Capacity ──────────────────────────────────────────────────────────────
    axes[0, 1].axhline(bl_cap_avg, color='y', linestyle='--', linewidth=1.5,
                       label=f'Baseline (avg = {bl_cap_avg:.2f})')
    axes[0, 1].plot(uav_range, r['Cap'],        'b-o',  markersize=3, label='General')
    axes[0, 1].plot(uav_range, r['Cap_wor'],   'r--',  markersize=3, label='Worst')
    axes[0, 1].plot(uav_range, r['Cap_bes'],   'g--',  markersize=3, label='Best')
    axes[0, 1].set_xlabel('Number of UAVs'); axes[0, 1].set_ylabel('Capacity (bits/s/Hz)')
    axes[0, 1].set_title(f'Increasing UAV {model_name} Channel Capacity'); axes[0, 1].legend(); axes[0, 1].grid(True)

    # ── Outage ────────────────────────────────────────────────────────────────
    axes[0, 2].axhline(bl_out_avg, color='y', linestyle='--', linewidth=1.5,
                       label=f'Baseline (avg = {bl_out_avg:.2f})')
    axes[0, 2].plot(uav_range, r['Out'],        'b-o',  markersize=3, label=' General')
    axes[0, 2].plot(uav_range, r['Out_wor'],   'r--',  markersize=3, label='Worst')
    axes[0, 2].plot(uav_range, r['Out_bes'],   'g--',  markersize=3, label='Best')
    axes[0, 2].set_xlabel('Number of UAVs'); axes[0, 2].set_ylabel('Outage Probability')
    axes[0, 2].set_title(f'Increasing UAV {model_name} Outage Probability'); axes[0, 2].legend(); axes[0, 2].grid(True)

    # ── BER ───────────────────────────────────────────────────────────────────
    axes[1, 0].axhline(bl_ber_avg, color='y', linestyle='--', linewidth=1.5,
                       label=f'Baseline (avg = {bl_ber_avg:.4f})')
    axes[1, 0].semilogy(uav_range, r['BER'],       'b-o',  markersize=3, label='General')
    axes[1, 0].semilogy(uav_range, r['BER_wor'],  'r--',  markersize=3, label='Worst')
    axes[1, 0].semilogy(uav_range, r['BER_bes'],  'g--',  markersize=3, label='Best')
    axes[1, 0].set_xlabel('Number of UAVs'); axes[1, 0].set_ylabel('BER')
    axes[1, 0].set_title(f'Increasing UAV {model_name} BER (log scale)'); axes[1, 0].legend(); axes[1, 0].grid(True, which='both')

    # ── Energy Efficiency ─────────────────────────────────────────────────────
    axes[1, 1].plot(uav_range, r['EE'],     'b-o', markersize=3, label='General')
    axes[1, 1].plot(uav_range, r['EE_wor'], 'r--', markersize=3, label='Worst')
    axes[1, 1].plot(uav_range, r['EE_bes'], 'g--', markersize=3, label='Best')
    axes[1, 1].set_xlabel('Number of UAVs'); axes[1, 1].set_ylabel('Energy Efficiency (bits/s/Hz/W)')
    axes[1, 1].set_title(f'Increasing UAV {model_name} Energy Efficiency'); axes[1, 1].legend(); axes[1, 1].grid(True)

    # ── Effective SINR handover ───────────────────────────────────────────────
    axes[1, 2].axhline(bl_avg, color='y', linestyle='--', linewidth=1.5,
                       label=f'Baseline (avg = {bl_avg:.2f})')
    axes[1, 2].plot(uav_range, eff_SINR, 'b-o', markersize=3,
                    label='Effective SINR (Baseline → General)')
    axes[1, 2].set_xlabel('Number of UAVs'); axes[1, 2].set_ylabel('Mean SINR (linear)')
    axes[1, 2].set_title(f'Increasing UAV {model_name} Effective SINR')
    axes[1, 2].legend(); axes[1, 2].grid(True)

    plt.savefig(rf'C:\Users\n70943sb\OneDrive - The University of Manchester\uni\Y3\Individual Project\Results\{model_name}_plots_incUAV.png')
    plt.tight_layout()
    plt.show()
...
def plot_model_comparison():
    ...
    fig.suptitle(f'General SINR Comparison Across Increasing UAV Models', fontsize=13)
    ...
    for model_name, r in results.items():
        ...
        ax.plot(uav_range, r['SINR'], color=s['color'], marker=s['marker'],
    ...
    plt.savefig(rf'C:\Users\n70943sb\OneDrive - The University of Manchester\uni\Y3\Individual Project\Results\combined_SINR_plot_incUAV.png')
    plt.tight_layout()
    plt.show()
