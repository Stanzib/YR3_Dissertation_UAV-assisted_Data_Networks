...
# -------- Each Power -----------------------------------------
for num_Power in range(1, num_Powers_total + 1):
    ...
    print("starting Power: ", num_Power, "\n")
    ...
    for j in range(num_iterations):
        ...
        d_uav_host = np.sqrt((Host_Pos_cart[0]-UAV_Pos_cart[0])**2 +
                         (Host_Pos_cart[1]-UAV_Pos_cart[1])**2 +
                         (Host_Pos_cart[2]-UAV_Pos_cart[2])**2)
        

        # ------ Hop 1: BS to UAV (Rician A2G)--------------
        d_bs_uav = np.sqrt((BS_Pos[servicing_BS][0] - UAV_Pos_cart[0])**2 +
                           (BS_Pos[servicing_BS][1] - UAV_Pos_cart[1])**2 +
                           (BS_Pos[servicing_BS][2] - UAV_Pos_cart[2])**2)
        
        signal1 = P_tx_BS * rician_gain(K_factor)[0] * path_loss(d_bs_uav, PathLossExponent_UAV)

        #______ interference from other BS to UAV______
        interf1 = sum(
            P_tx_BS * rician_gain(K_factor)[0] * path_loss(
                np.sqrt((UAV_Pos_cart[0]-u[0])**2 +
                        (UAV_Pos_cart[1]-u[1])**2 +
                        (UAV_Pos_cart[2]-u[2])**2), PathLossExponent_UAV)
        ...
        #____ interferance from other BS to Host_____
        interf2 = sum(
            P_tx_BS * rayleigh_gain() * shadowing_gain() * path_loss(
        ...
        P_total_UAV  = P_tx_BS + num_Power * (P_tx_UAV + P_UAV_flight)
...
def print_results():
    power_range = range(1, num_Powers_total + 1)

    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle('UAV Relay Performance Metrics vs Power', fontsize=14)

    # ── SINR ─────────────────────────────────────────────────────────────────
    axes[0, 0].plot(power_range, SINR_list, 'b-o', markersize=3)
    axes[0, 0].plot(power_range, SINR_list_BL, 'r-o', markersize=3)
    ...
    # ── Capacity ──────────────────────────────────────────────────────────────
    axes[0, 1].plot(power_range, Capacity_list, 'b-o', markersize=3)
    axes[0, 1].plot(power_range, Capacity_list_BL, 'r-o', markersize=3)
    ...
    # ── Outage ────────────────────────────────────────────────────────────────
    axes[0, 2].plot(power_range, Outage_list, 'b-o', markersize=3)
    axes[0, 2].plot(power_range, Outage_list_BL, 'r-o', markersize=3)
    axes[0, 2].set_xlabel('Power')
    axes[0, 2].set_ylabel('Outage Probability')
    axes[0, 2].set_title('Outage Probability')
    axes[0, 2].grid(True)

    # ── BER ───────────────────────────────────────────────────────────────────
    axes[1, 0].semilogy(power_range, BER_list, 'b-o', markersize=3)
    axes[1, 0].semilogy(power_range, BER_list_BL, 'r-o', markersize=3)
    ...
    # ── Energy Efficiency ─────────────────────────────────────────────────────
    axes[1, 1].plot(power_range, Energy_Eff_list, 'b-o', markersize=3)
    axes[1, 1].plot(power_range, Energy_Eff_list_BL, 'r-o', markersize=3)
...
def print_test_1():
    plt.plot(range(1, num_Powers_total + 1), Outage_list, 'b-')
    plt.xlabel('Power in dead zone')
...
def print_test_2():
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(1, num_Powers_total + 1), SINR_list, 'r-')
    plt.xlabel('Power in dead zone')
