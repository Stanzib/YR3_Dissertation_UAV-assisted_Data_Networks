...
def performance_metrics(sinr, is_uav):
        ...
        if is_uav == True:
            ...
            Capacity = np.log2(1 + sinr) # half-duplex penalty
        Energy_Eff = Capacity / P_total
...
for num_UAV in range(1, num_UAVs_total + 1):
    ...
    for j in range(num_iterations):
        ...
        for _ in range(num_UAV):
            UAV_pos_list.append(random_pos(True))
        ...
        signal3 = P_tx_BS * rayleigh_gain() * path_loss(d_bs_host, PathLossExponent_urban)
        ...
        P_total  = P_tx_BS + num_UAV * (P_tx_UAV + P_UAV_flight)

        #---------- Calculating performance metrics--------------
        SINR_e2e = e2e_sinr(SINR1, SINR2, relay_mode)

        performance_metrics(SINR_e2e, True)
