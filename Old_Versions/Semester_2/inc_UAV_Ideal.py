...
print("script started: Increasing UAV, Ideal Model")
...
def print_results():
    ...
    # Build the combined line: baseline until UAV SINR exceeds it, then UAV SINR
    combined = [max(s, baseline_average) for s in SINR_list]
    axes[1, 2].axhline(y=baseline_average, color='y', linestyle='--', linewidth=1.5,
               label=f'Baseline (no UAV, avg = {baseline_average:.2f})')
    axes[1, 2].plot(uav_range, combined, 'b-o', markersize=3,
            label='Effective SINR (baseline until UAV relay exceeds it)')
    axes[1, 2].set_xlabel('Number of UAVs')
    axes[1, 2].set_ylabel('Mean SINR (linear)')
    axes[1, 2].set_title('Effective SINR — Baseline Handover to UAV Relay')
    axes[1, 2].legend()
    axes[1, 2].grid(True)
...
baseline_average = float(np.squeeze(sinr_basline_total / (num_iterations * num_UAVs_total)))
