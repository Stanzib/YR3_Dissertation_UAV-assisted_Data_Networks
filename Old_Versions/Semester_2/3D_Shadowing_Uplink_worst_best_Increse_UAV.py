...
for num_UAV in range(1, num_UAVs_total + 1):
    ...
    for j in range(num_iterations):
        ...
        singal_baseline = P_tx_Host * rayleigh_gain() * shadowing_gain() * path_loss(d_bs_host, PathLossExponent_urban)
        ...
        interf_baseline = sum(
            P_tx_Host * rayleigh_gain() * shadowing_gain() * path_loss(
                np.sqrt((BS_Pos[servicing_BS][0]-u[0])**2 +
                        (BS_Pos[servicing_BS][1]-u[1])**2 +
                        (BS_Pos[servicing_BS][2]-u[2])**2), PathLossExponent_urban)
            for u in interf_rand_pos_list
        )
        ...
        SINR_e2e_baseline = singal_baseline / (interf_baseline + Noise())
        ...
        performance_metrics(SINR_e2e_baseline, True, acc_baseline)
...
def print_test_1():
    plt.plot(range(1, num_UAVs_total + 1), Outage_list, 'b-')
...
def print_test_2():
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(1, num_UAVs_total + 1), SINR_list, 'r-')
    plt.xlabel('Number of UAVs in dead zone')
