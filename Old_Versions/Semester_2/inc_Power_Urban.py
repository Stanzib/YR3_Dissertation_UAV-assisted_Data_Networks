...
# We increase the Power transmitted by Hosts
# Uplink
# 6 interfering BS
# best case worst case calculated
print("script started: Increasing Host power trsnsmit, Urban Model")
...
PathLossExponent_UAV = 3 # free space
...
def shadowing_gain(angle_deg=None):
    ...
    #     sigma_dB = 8.0
    # else:
    #     sigma_dB = 8.0 - (6.0 * (angle_deg / 90.0))
    #     sigma_dB = np.clip(sigma_dB, 2.0, 8.0)

    # X_sigma_dB = np.random.normal(0, sigma_dB)
    # return 10 ** (X_sigma_dB / 10)
    ...
    if angle_deg is None:
        ...
        # At 0° (horizontal, near-NLoS) -> sigma = 12 dB  (upper bound)
        # At 90° (overhead, strong LoS)  -> sigma = 4 dB   (lower bound)
...
# -------- Each UAV -----------------------------------------
for i in range(1, Iteration_range + 1):
    ...
    P_tx_Host = i * power_increment
    power_values.append(P_tx_Host)
    ...
    for j in range(num_iterations):
        ...
        #baseline interf
        fading_shadow_interf_baseline = [rayleigh_gain() * shadowing_gain(angles_interf_to_BS_general[i]) 
                for i, u in enumerate(interf_rand_pos_list)]
        #baseline desired sig
        fading_shadow_desired_baseline = rayleigh_gain() * shadowing_gain(elevation_angle(BS_Pos[servicing_BS], Host_Pos_cart))
        ...
        if num_UAV == 50:  # just check one UAV count
...
def print_results():
    ...
    fig.suptitle('UAV Relay Performance vs Host Transmit Power (Urban Environment)', fontsize=14)

    # ── SINR ─────────────────────────────────────────────────────────────────
    axes[0, 0].plot(x_vals, SINR_list, 'b-o', markersize=3, label='General')
    axes[0, 0].plot(x_vals, SINR_list_wor, 'r--', markersize=3, label='Worst Case')
    axes[0, 0].plot(x_vals, SINR_list_bes, 'g--', markersize=3, label='Best Case')
    axes[0, 0].set_xlabel('Host Transmit Power (W)')
    axes[0, 0].set_ylabel('Mean SINR (Linear)')
    axes[0, 0].set_title('SINR vs Host Power')
    axes[0, 0].legend()
    axes[0, 0].grid(True)

    # ── Capacity ──────────────────────────────────────────────────────────────
    axes[0, 1].plot(x_vals, Capacity_list, 'b-o', markersize=3, label='General')
    axes[0, 1].plot(x_vals, Capacity_list_wor, 'r--', markersize=3, label='Worst Case')
    axes[0, 1].plot(x_vals, Capacity_list_bes, 'g--', markersize=3, label='Best Case')
    axes[0, 1].set_xlabel('Host Transmit Power (W)')
    axes[0, 1].set_ylabel('Capacity (bits/s/Hz)')
    axes[0, 1].set_title('Capacity vs Host Power')
    axes[0, 1].grid(True)

    # ── Outage ────────────────────────────────────────────────────────────────
    axes[0, 2].plot(x_vals, Outage_list, 'b-o', markersize=3, label='General')
    axes[0, 2].plot(x_vals, Outage_list_wor, 'r--', markersize=3, label='Worst Case')
    axes[0, 2].plot(x_vals, Outage_list_bes, 'g--', markersize=3, label='Best Case')
    axes[0, 2].set_xlabel('Host Transmit Power (W)')
    axes[0, 2].set_ylabel('Outage Probability')
    axes[0, 2].set_title('Outage vs Host Power')
    axes[0, 2].grid(True)

    # ── BER ───────────────────────────────────────────────────────────────────
    axes[1, 0].semilogy(x_vals, BER_list, 'b-o', markersize=3, label='General')
    axes[1, 0].semilogy(x_vals, BER_list_wor, 'r--', markersize=3, label='Worst Case')
    axes[1, 0].semilogy(x_vals, BER_list_bes, 'g--', markersize=3, label='Best Case')
    axes[1, 0].set_xlabel('Host Transmit Power (W)')
    axes[1, 0].set_ylabel('Bit Error Rate (BER)')
    axes[1, 0].set_title('BER vs Host Power (Log Scale)')
    axes[1, 0].grid(True, which='both')

    # ── Energy Efficiency ─────────────────────────────────────────────────────
    axes[1, 1].plot(x_vals, Energy_Eff_list, 'b-o', markersize=3, label='General')
    axes[1, 1].plot(x_vals, Energy_Eff_list_wor, 'r--', markersize=3, label='Worst Case')
    axes[1, 1].plot(x_vals, Energy_Eff_list_bes, 'g--', markersize=3, label='Best Case')
    axes[1, 1].set_xlabel('Host Transmit Power (W)')
    axes[1, 1].set_ylabel('Energy Efficiency (bits/s/Hz/W)')
    axes[1, 1].set_title('Energy Efficiency vs Host Power')
    axes[1, 1].grid(True)

    # ── Baseline SINR ─────────────────────────────────────────────────────────
    axes[1, 2].plot(x_vals, SINR_list_baseline, 'y-o', markersize=3, label='Direct Link (No UAV)')
    axes[1, 2].set_xlabel('Host Transmit Power (W)')
    axes[1, 2].set_ylabel('Mean SINR (Linear)')
    axes[1, 2].set_title('Baseline SINR vs Host Power')
    axes[1, 2].legend()
    axes[1, 2].grid(True)
...
baseline_average = sinr_basline_total / (num_iterations*Iteration_range)
